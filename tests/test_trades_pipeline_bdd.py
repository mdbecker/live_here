...
class TradesPipelineBehaviors(unittest.TestCase):
    def test_given_normalized_trades_source_when_pipeline_runs_then_factor_is_rankable(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            counties = root / "counties.csv"; counties.write_text("fips,name,state,geography_vintage\n01001,Alpha,AL,2019\n01003,Beta,AL,2019\n")
            ...
            config = {"mode":"research","geography_source":"counties","factors":["tradespeople"],"missing_policy":"complete_case","iterations":10,"seed":42,
