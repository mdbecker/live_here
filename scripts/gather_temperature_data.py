...
def main():
    ...
    receipt = download(NOAA_URL, raw_dir / NOAA_FILE, fetch=fetch)
    if args.download_only:
        print(json.dumps({"noaa": receipt}, indent=2))
        return
    ...
    audit.update({"county_rows": len(counties), "counties_with_temperature_metrics": len(values),
                  "raw_sources": {"noaa": receipt}, "method": "station expected TMAX days; local equirectangular IDW inverse-square interpolation within 125 km"})
    ...
    if config_path.exists():
        ...
        config["sources"].append({"id": "temperature", "path": "temperature.csv", "sha256": sha256(target / "temperature.csv"),
                                   ...
                                   "raw_parent_sha256": {"noaa": receipt["sha256"], **county_parents},
                                   "raw_parent_urls": {"noaa": receipt["url"], **county_parent_urls}})
